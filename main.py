...
async def start(message):
    ...
    if is_verified(user_id):
        ...
        await bot.reply_to(message, "🔐 مرحبًا بك.\nأرسل رقم هاتفك مع رمز الدولة:\nمثال: +9647701234567")
...
async def handle_phone(message):
    ...
    if result is True:
        await bot.reply_to(message, "✅ تم إرسال كود التفعيل إلى حسابك.\nأدخل الكود بمسافات:\nمثال: 1 2 3 4 5")
    else:
...
async def handle_code(message):
    ...
    if result is True:
        ...
        await bot.reply_to(message, f"❌ كود خاطئ.\nاستخدم /login مرة أخرى")
...
async def handle_password(message):
    ...
    if result is True:
        ...
        await bot.reply_to(message, "❌ كلمة مرور خاطئة.")
...
async def takleesh(message):
    ...
    await bot.reply_to(message, "✍️ أرسل معرف المستخدم المستهدف:")
...
async def tasteer(message):
    ...
    if not is_verified(user_id):
        await bot.reply_to(message, "❌ يجب تسجيل الدخول أولاً.")
        return
...
async def get_target_tasteer(message):
    target = message.text.strip()
    await bot.reply_to(message, "⏱️ السرعة بين كل سطر (ثواني، مثال: 3):")
